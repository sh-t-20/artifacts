...
//		int[] product_order = {1, 2, 12, 13, 4, 14, 5, 11, 3, 8, 9, 10, 6, 7};
		...
		// An efficient order for BCS
		int[] product_order = { 1, 3, 4, 5, 2 };
...
//			String product_selection_method = "feature_coverage";
			...
			SimpleConfiguration config_1 = GetConfig(productIndex_1, products_dir);
...
//			System.out.println(product_features.toString());
			...
			statistics_adaptive = LearnFSM(productFile_1, ot_list, learningAlgorithm, out_dir, line, rnd_seed,
					statistics_adaptive, 1, logger_1, config_1);
			statistics_nonadaptive = LearnFSM(productFile_1, ot_list, learningAlgorithm, out_dir, line, rnd_seed,
					statistics_nonadaptive, 0, logger_1, config_1);

			System.out.println(ot_list.size());

			for (int i = 0; i < (selected_products.length - 1); i++) {
				int productIndex_2 = NextProductIndex(selected_products, product_selection_method, products_similarity,
						product_order, configs_properties);
				SimpleConfiguration config_2 = GetConfig(productIndex_2, products_dir);
...
//				System.out.println(product_features.toString());
				...
				statistics_adaptive = LearnFSM(productFile_2, ot_list, learningAlgorithm_2, out_dir, line, rnd_seed,
						statistics_adaptive, 1, logger_1, config_2);

				statistics_nonadaptive = LearnFSM(productFile_2, ot_list, learningAlgorithm_3, out_dir, line, rnd_seed,
						statistics_nonadaptive, 0, logger_1, config_2);
	...
	private static SimpleConfiguration GetConfig(int productIndex, File products_dir_1) {
		// TODO Auto-generated method stub
		...
		try {
			config = FtsUtils.getInstance().loadConfiguration(config_string);
			return config;
		} catch (IOException e) {
...
//					System.out.println(k +1);
	...
	private static int[] LearnFSM(File sul_file, ArrayList<ObservationTable> otList, String learningAlgorithm_1,
			...
			LearnLogger logger, SimpleConfiguration config) {
		try {
